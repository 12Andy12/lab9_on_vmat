...
int n = 5;
double y[5] = { 1,5,1,-3,1 };
double x[5] = { 1,5,9,13,17 };
int h = 4;
int X = 12;
...
double Trig(double X)
{
	...
	for (int i = 1; i < n; i++)
	{
		...
		for (int j = 0; j < n - 1; j++)
		{
			bj += y[j] * cos(2 * PI * i * j / n) / n;
			aj += y[j] * sin(2 * PI * i * j / n) / n;
		}
		res += bj * cos(2 * PI * i * (X - x[0]) / (n * h)) + aj * sin(2 * PI * i * (X - x[0]) / (n * h));
	}
	res += b0;
	--n;
	return res;
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		sf::VertexArray grafic(sf::LinesStrip, (x[n - 1]) * 10 + 1);
